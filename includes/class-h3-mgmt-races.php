...
class H3_MGMT_Races {
	...
	public function donation_counter( $atts ) {
		...
		list( $sponsors, $sponsor_counts, $donations ) = $h3_mgmt_sponsors->get_sponsors_meta( array( 'parent' => $race, 'exclude_unpaid' => true ) );

		$min = 0;
		$current = isset( $donations['thumbs'] ) ? $donations['thumbs'] : 1;
		$max = 20000;
		...
		$output .= '>' .
					...
					'<div id="counter-value-wrap">' .
						'<span id="counter-value-no-js" class="no-js-toggle">' . $current . '</span>' .
						'<span id="counter-value" class="js-toggle">' . $min . '</span>' .
						'<br /><span class="counter-thumbs">' . _x( 'Thumbs', 'Donation Counter', 'h3-mgmt' ) . '</span>' .
						'<br /><span class="counter-euros">' . _x( '(10 Euros each)', 'Donation Counter', 'h3-mgmt' ) . '</span>' .
					'</div>' .
				'</div>' .
			'</div>';
	...
	public function ranking_submission_form( $route_id, $stage_number, $message = '' ) {
		...
		foreach( $teams as $team ) {
			$output .= '<div class="form-row">' .
					'<h3>' . $team['team_name'] . '</h3><br />' .
					'<label>' . __( 'Rank', 'h3-mgmt' ) . '</label>&nbsp;&nbsp;' .
					'<select id="rank-' . $team['id'] . '" name="rank-' . $team['id'] . '">';
		...
		$output .= '<input type="submit" value="' . _x( 'Save Ranking', 'Ranking Submission', 'h3-mgmt' ) . '"></form>';
	...
	public function ranking_table( $atts='' ) {
			...
			$output .= '<div class="isotope-wrap">' .
					'<ul class="isotope-link-list">' .
						'<li><a href="' . get_site_url() .
									_x( '/follow-us/rankings/', 'utility translation', 'h3-mgmt' ) .
		...
		$output .= '<table class="ranking padded-table" cellspacing="0">'.
		...
		foreach( $teams as $team ) {
			...
			$output .= '<tr class="trow-alt-' . $flip_val . '">' .
				'<th><span style="font-weight:700;border-bottom: 2px solid #' . $this->get_route_color( $team['route_id'] ) . ';">' .
						'<a class="inkognito-link" title="' . __( 'View TeamProfile', 'h3-mgmt' ) . '" href="http://' . __( 'tramprennen.org/follow-us/teams/?id=', 'h3-mgmt' ) . $team['id'] . '">' .
							$team['team_name'] .
						'</a>' .
					'</span>' .

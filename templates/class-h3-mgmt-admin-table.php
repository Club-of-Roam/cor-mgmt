...
if ( ! class_exists( 'H3_MGMT_Admin_Table' ) ) :
	...
	class H3_MGMT_Admin_Table {
		...
		public function __construct( $args, $columns, $rows ) {
			...
			if ( isset( $_GET['order'] ) && ( 'ASC' === $_GET['order'] || 'DESC' === $_GET['order'] ) ) {
				...
				if ( 'ASC' === $this->args['order'] ) {
					$this->args['toggle_order'] = 'DESC';
				} else {
					$this->args['toggle_order'] = 'ASC';
				}
			}
		...
		public function output() {
			...
			if ( isset( $this->set_args['pre_filtered'][0] ) && true === $this->set_args['pre_filtered'][0] &&
			! isset( $_GET['filter_name0'] ) && is_array( $rows ) && ! empty( $rows ) ) {
			...
			if ( $this->args['with_wrap'] ) {
				$output .= '<div class="wrap">' .
				'<div id="' . $this->args['icon'] . '" class="icon32"><br></div>' .
				'<h2>' . $this->args['headline'] . '</h2><br />';
			}

			if ( $this->args['headspace'] ) {
				$output .= '<br />';
			}
			...
			(
				$this->args['with_bulk'] ||
				( ! $this->args['with_bulk'] && ! $this->args['dspl_cnt'] && ! $this->args['pagination'] )
			)
			) {
				$output .= '<h3 class="table-headline">';
				if ( isset( $this->set_args['icon'] ) && ! empty( $this->set_args['icon'] ) ) {
					$output .= '<div class="tbl-icon nt-' . $this->args['icon'] . '"></div>';
				}
				$output .= $this->args['headline'] . '</h3>';
			}
			...
			if ( ! empty( $filter ) ) {
				...
				foreach ( $filter as $filter_is ) {
					...
					if ( isset( $_GET[ 'filter_value' . $filter_id ] ) && '-1' != $_GET[ 'filter_value' . $filter_id ] && 'all' != $_GET[ 'filter_value' . $filter_id ] ) {
					...
					if ( isset( $_GET[ 'filter_value' . $filter_id ] ) && 'all' != $_GET[ 'filter_value' . $filter_id ] ) {
			...
			if ( empty( $rows ) ) {
				...
				if ( $this->args['show_empty_message'] ) {
					if ( ! empty( $this->args['empty_message'] ) ) {
						$message = $this->args['empty_message'];
					} elseif ( ! empty( $this->args['data_name'] ) ) {
						$message = sprintf( __( 'Currently there is no data of type &quot;%s&quot;...', 'h3-mgmt' ), $this->args['data_name'] );
					} else {
						$message = __( 'Currently there is no data to be displayed here...', 'h3-mgmt' );
					}

					$output .= $h3_mgmt_admin->convert_messages(
						array(
							array(
								'type'    => 'message',
								'message' => $message,
				...
				if ( $this->args['with_bulk'] && is_array( $this->args['bulk_actions'] ) && ! empty( $this->args['bulk_actions'] ) ) {
				...
				foreach ( $columns as $column ) {
					...
					if ( isset( $column['sortable'] ) && true === $column['sortable'] ) {
						...
						if ( $column['id'] !== $this->args['orderby'] ) {
							$col_order        = 'DESC';
							$col_toggle_order = 'ASC';
							/* sorting if this is the column currently sorted by */
						} else {
							$col_order        = $this->args['order'];
							$col_toggle_order = $this->args['toggle_order'];
						}
				...
				if ( $this->args['with_bulk'] && is_array( $this->args['bulk_actions'] ) && ! empty( $this->args['bulk_actions'] ) ) {
					...
					'<input type="hidden" name="page" value="' . $this->args['page_slug'] . '" />';
					if ( ! empty( $this->args['extra_bulk_html'] ) ) {
						$output .= $this->args['extra_bulk_html'];
					}
				}

				if (
				( $this->args['with_bulk'] && is_array( $this->args['bulk_actions'] ) && ! empty( $this->args['bulk_actions'] ) ) ||
				$this->args['dspl_cnt'] ||
				...
				if ( $this->args['with_bulk'] && is_array( $this->args['bulk_actions'] ) && ! empty( $this->args['bulk_actions'] ) ) {
					...
					if ( ! empty( $this->args['bulk_desc'] ) ) {
						$output .= '<span class="desc">' . $this->args['bulk_desc'] . ':&nbsp;</span>';
					}
					if ( 1 < count( $this->args['bulk_actions'] ) ) {
						$output .= '<select name="' . $this->args['bulk_param'] . '" id="' . $this->args['bulk_param'] . '" class="bulk-action simul-select">';
						foreach ( $this->args['bulk_actions'] as $bulk_action ) {
							$output .= '<option value="' . $bulk_action['value'] . '">' . $bulk_action['label'] . '&nbsp;</option>';
						}
						$output .= '</select>';
					} else {
						$bulk_action = $this->args['bulk_actions'][0];
						$output     .= '<input type="hidden" name="' . $this->args['bulk_param'] . '" value="' . $bulk_action['value'] . '" />';
					}
					$output .= '<input type="submit" name="" id="bulk-action-submit" class="button-secondary do-bulk-action" value="' .
						$this->args['bulk_btn'] . '"';
					if ( ! empty( $this->args['bulk_confirm'] ) ) {
						$output .= ' cc="ccc"';// onclick="if ( confirm(\'' . $this->args['bulk_confirm'] . '\') ) { return true; } return false;"';
					}
					$output .= '/></div>';
				} elseif (
				! $this->args['with_wrap'] &&
				isset( $this->set_args['headline'] ) &&
				! empty( $this->set_args['headline'] ) &&
				( $this->args['dspl_cnt'] || $this->args['pagination'] )
				) {
					$output .= '<div class="alignleft"><h3 class="table-headline">';
					if ( isset( $this->set_args['icon'] ) && ! empty( $this->set_args['icon'] ) ) {
						$output .= '<div class="tbl-icon nt-' . $this->args['icon'] . '"></div>';
					}
					$output .= $this->args['headline'] . '</h3></div>';
				}

				if ( $this->args['dspl_cnt'] || $this->args['pagination'] ) {
					$output .= '<div class="tablenav-pages">';
					if ( $this->args['dspl_cnt'] ) {
						$output .= '<span class="displaying-num">' . sprintf( $this->args['cnt_txt'], $count ) . '</span>';
					}
					if ( $this->args['pagination'] ) {
						$pagination_html = paginate_links(
							array(
								'base'      => $this->args['pagination_url'],
								'format'    => '&p=%#%#tbl',
								'prev_text' => $this->args['prev_text'],
								'next_text' => $this->args['next_text'],
								'total'     => $this->args['total_pages'],
								'current'   => $this->args['current_page'],
				...
				foreach ( $rows as $row ) {
					...
					if ( $this->args['with_bulk'] && is_array( $this->args['bulk_actions'] ) && ! empty( $this->args['bulk_actions'] ) ) {
						...
						$output  .= '<th class="check-column" scope="row">' .
							// '<input type="checkbox" name="' . $this->args['bulk_name'] . '" value="' . $bulk_val . '">' .
							'<input type="checkbox" name="' . $this->args['bulk_name'] . '[' . $bulk_id . ']" value="' . $bulk_val . '">' .
						'</th>';
						$bulk_id = $bulk_id + 1;
					}
				...
				if ( $this->args['with_bulk'] && is_array( $this->args['bulk_actions'] ) && ! empty( $this->args['bulk_actions'] ) ) {
					...
					$output .= '<input type="submit" name="" id="bulk-action-submit" class="button-secondary do-bulk-action" value="' .
						$this->args['bulk_btn'] . '"';
					if ( ! empty( $this->args['bulk_confirm'] ) ) {
						$output .= ' onclick="if ( confirm(\'' . $this->args['bulk_confirm'] . '\') ) { return true; } return false;"';
					}
					$output .= '/></div>';
				}
		...
		public function get_filtered_rows( $filter, $rows ) {
			...
			if ( isset( $_GET['filter_name0'] ) ) {
				foreach ( $filter as $filter_is ) {
					if ( '' != $_GET[ 'filter_value' . $filter_id ] && '-1' != $_GET[ 'filter_value' . $filter_id ] && 'all' != $_GET[ 'filter_value' . $filter_id ] ) {
						$rows_new = array();
						foreach ( $rows as $row_is ) {
